...
from src.rl.algorithms.monte_carlo import monte_carlo_es
from src.rl.environments.grid_world import DOWN, GridWorldEnv
from src.rl.environments.line_world import LineWorldEnv


def test_monte_carlo_es_hand_verifiable_on_smallest_line_world():
    ...
    pi, Q = monte_carlo_es(env, iterations_count=2_000)
...
def test_monte_carlo_es_terminal_states_have_no_action():
    np.random.seed(0)
    env = LineWorldEnv(num_cells=5)
    pi, Q = monte_carlo_es(env, iterations_count=5_000)

    for s in env.terminal_states():
        assert pi[s].sum() == 0.0


def test_monte_carlo_es_optimal_policy_always_goes_right_in_line_world():
    np.random.seed(0)
    env = LineWorldEnv(num_cells=5)
    pi, Q = monte_carlo_es(env, iterations_count=8_000)

    for s in range(1, env.num_cells - 1):
        assert pi[s, 1] == 1.0


def test_monte_carlo_es_hand_verifiable_on_smallest_grid_world():
    ...
    pi, Q = monte_carlo_es(env, iterations_count=3_000)
